 ...
 public:
  ...
  explicit DmlCropAndResizeKernel(DmlKernelConstruction* ctx,
    ...
    tensors.inputs[0]->desc =
        CreateTensorDescFromInput(ctx, 0, DmlTensorLayout::Nhwc());

    // RoiAlign only supports uint32 for the batch indices
    tensors.inputs[2]->desc.ForceUnsignedDataType();

    tensors.outputs[0]->desc =
        CreateTensorDescFromOutput(ctx, 0, DmlTensorLayout::Nhwc());

    auto inputs = GetDmlTensorDescs(tensors.inputs);
    auto scope = dml::Scope(ctx->GetDmlDevice(), dml::TensorLayout::Nhwc);
    ...
    constexpr float pixelCenter = 0.0f;
    constexpr int32_t sampleEndAdjustment = -1;
    constexpr uint32_t minimumSamplesPerOutput = 1;
    constexpr uint32_t maximumSamplesPerOutput = 1;

    auto result = dml::RoiAlign(
        input, roi, batch_indices, DML_REDUCE_FUNCTION_MAX,
        ...
        maximumSamplesPerOutput, crop_height, crop_width, pixelCenter,
        sampleEndAdjustment);
